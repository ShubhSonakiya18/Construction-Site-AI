...
- Run the full suite before considering any change done: `pytest tests/ -q`. Current baseline: **930 passed, 0 skipped** (the live-Groq extraction test now runs — `tests/conftest.py` loads `.env` — instead of being permanently skipped; see `docs/DECISIONS.md` for why that mattered).
- **UI-facing changes (Sprint 18+)**: extend `frontend/e2e/` with a real Playwright spec rather than a fresh, throwaway verification script — the same discipline a backend change already follows by extending `tests/` instead of a one-off script. See `docs/E2E_TESTING.md` for how to run the suite and its real login-rate-limit gotcha (reuse `auth.setup.ts`'s shared authenticated session; don't log in fresh per test). A one-off script is still fine for something genuinely exploratory that won't be re-run.
