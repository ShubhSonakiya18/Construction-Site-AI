...
**Deliverable 3 confirmation:** ran the same Spanish transcript through `AIServiceManager.generate_all()` (real Groq, `DailyReportService`) after the prompt fix, with no further code changes. Produced a complete, fully English, professionally-formatted daily report — correct stage, workforce count, trade, weather, and the near-miss incident narrative, all in English — confirming the fix holds all the way to a finished document, not just at the extraction step. Regression-tested directly against the extraction layer in `tests/test_extraction_pipeline.py::TestRealGroqEngine::test_spanish_transcript_extracts_english_free_text` (real-Groq-gated, `HAS_GROQ` skip) rather than re-running full document generation on every test run, since the extraction layer is where the actual translation happens and generation only ever sees its already-English output.

---

## Known Bug Found and Fixed — Sprint 16, Deliverable 4 (2026-09-17)

**`SPEECH_WHISPER_LANGUAGE=en` in the real, deployed `.env` (and documented as the default in `.env.example`) silently disabled this entire sprint's feature.** `speech/config.py`'s `SpeechProcessingConfig.from_env()` reads this variable and, when set to a fixed code, forces Whisper to transcribe *every* file as if it were that language — not merely skip auto-detection, but actively mis-transcribe. Confirmed by running the real Spanish audio sample (`data/sample_audio/sample_11_spanish_foreman.wav`) through the actual production code path (`app/services/pipeline_service.py`'s `run_pipeline()`, which calls `SpeechProcessingConfig.from_env()` — not the bare `SpeechProcessingConfig()` every prior verification in ADR-064 had used) via a real `POST /audio/upload` against the live API: the persisted transcript was *"Good morning! Today we worked on the stage in the market. We had six workers in the city... The climb was heavy all the day. A carpenter called Miguel is about to go out, but not the top. It was an accident."* — grammatically plausible but semantically wrong English, `language_code="en"`, `language_probability=1.0` (Whisper reporting full confidence in a language it was never allowed to question). ADR-064's own verification had used a bare `SpeechProcessingConfig()` (correctly bypassing `.env`), which is why the bug wasn't caught until Deliverable 4's live end-to-end check against the real upload endpoint.

**Fix:** `SPEECH_WHISPER_LANGUAGE=` (empty, i.e. auto-detect) in both `.env` (the live deployment config) and `.env.example` (the documented default), with `.env.example`'s comment rewritten to state plainly what setting a fixed code actually does — not "skip detection" but "force every file to be mis-transcribed as this language if it's actually something else." Re-ran the identical Spanish audio file through the identical real code path after the fix: `language_code="es"`, `language_probability=0.9825`, and the generated daily log correctly showed `current_stage="framing"`, `total_workers_present=6`, `task_description="Installed second floor walls"` — the real production pipeline, not just a direct-call test harness, now works end-to-end.

**Consequence:** Every verification claim in ADR-064 remains true for the *code* (the extraction-prompt English-output rule genuinely works, proven against real Groq) but was incomplete as a claim about the *deployed system*, since a config value outside the code paths tested had silently defeated the feature at the transcription stage before any of that code ever ran. This is a reminder specific to this project's own discipline: "live-verify, don't assume" needs to mean verifying the actual deployed configuration a real user's request would hit, not only the code path a direct function call exercises — the two silently diverged here because `.env` isn't loaded by a bare `SpeechProcessingConfig()` construction, only by `.from_env()`.

---

## ADR-065: Reference-Cost Estimator Is Materials-Only — No Labor-Hour Estimation (Sprint 17)

**Date:** Sprint 17, Deliverable 1
**Status:** Accepted

**Context:** Both remaining Phase 5 roadmap items (Defect Detection, Bid Estimation) were investigated before Sprint 17 was scoped and found genuinely blocked — Defect Detection has no photo-upload infrastructure or verified vision-capable model anywhere in this codebase; Bid Estimation's own roadmap framing ("historical project data → bid estimate") is unsupportable with the single project that exists in the database. The direction chosen instead: a deterministic reference-cost range estimator built on real existing data (`knowledge/construction_ontology.json`'s material cost ranges, `knowledge/dependency_graph.json`'s stage structure already consumed by Sprint 11's real `ScheduleTask` rows, and `Project.project_size_sqft`). Building this required one new piece of reference data that doesn't exist anywhere yet: a typical-quantity-of-material-per-square-foot figure for each cost-relevant stage. The same question arose for labor: should the estimator also include typical labor hours per stage, scaled the same way?

**Decision:** No — the estimator is materials-only. Unlike material cost ranges (real data already present in `construction_ontology.json`, entered when that ontology was first built in Sprint 1) and stage durations (real data in `dependency_graph.json`, likewise Sprint-1-era and already load-bearing for Sprint 11's real scheduling), there is no comparable reference figure for labor hours or labor rates anywhere in this codebase to build from. Inventing one from nothing — a "typical crew-hours per 1,000 sqft" number with no traceable basis — would be indistinguishable from a fabricated figure presented with the same false confidence this sprint exists to avoid (the same concern that ruled out Bid Estimation's original historical-data framing in the first place). A materials-only estimate, honestly labeled as such, is preferable to a materials-plus-labor estimate where half the number is unfounded.

**Consequence:** `knowledge/cost_estimation_reference.json` (new, Sprint 17) covers only the 9 stages that already have ontology-defined materials with real cost ranges (foundation, concrete_flatwork, framing, roofing, electrical_rough_in, plumbing_rough_in, hvac_rough_in, drywall, painting) — the other 14 dependency-graph stages (site_preparation, tile_work, cabinets_and_countertops, punch_list, inspection, etc.) are out of scope for cost estimation entirely, not assigned a fabricated figure. `app/services/cost_estimation_service.py`'s output and every UI label must describe the result as a "materials reference-cost estimate," never an all-in project cost or a bid — a real gap a future sprint could close only if a defensible labor-rate reference source is identified, not by guessing one now.

---

## ADR-066: Alert Dedup via a Per-(Project, Alert-Type) Last-Sent Table, Not a Status-Change Diff (Sprint 19)

**Date:** Sprint 19, Deliverable 1
**Status:** Accepted

**Context:** Sprint 14's `budget_variance.status` and Sprint 15's `safety_proactive_warnings` are both real, correct, already-computed alert conditions — but both sprints' own "Decided, not built" sections stopped short of pushing them anywhere, citing "no scheduler or notification infrastructure." That's no longer true (Sprint 9 built Celery + Redis + a real `EmailSender`), so Sprint 19 makes them genuinely proactive. The real design question: a periodic Celery Beat task that queries `compute_budget_variance()`/the safety warning functions on a fixed interval needs some way to avoid re-sending the identical alert every single tick forever — the naive version would email a company every hour for as long as a project stays `over_budget`, which is not an alert, it's spam. Two designs were considered: (a) diff the *previous computed status* against the *current computed status* on each tick, firing only on a transition (e.g. `on_track` → `over_budget`), keeping no independent "last sent" record at all; (b) a small new table recording, per (project_id, alert_type), the last status value sent and when, with a cooldown window independent of whether the status literally changed.

**Decision:** (b) — a new `project_alerts_sent` table (migration `009`), not a pure status-diff. A pure diff (option a) has no way to represent "the status has been `over_budget` for three weeks straight and nobody has been reminded" — a real, useful case for a budget alert (a one-time notification when a project first goes over budget, then silence for the rest of the project's life, undersells what "proactive" should mean for something this consequential). The table stores `last_status_value` (so a real transition, e.g. `approaching_budget` → `over_budget`, always fires regardless of cooldown — worse news should never be suppressed by a timer) and `last_sent_at` (so the *same* bad status re-fires only after a cooldown, not on every tick) together, which a pure diff cannot express without inventing the same state anyway. A 24-hour cooldown is the chosen default — a convention, not a derived figure, matching how `APPROACHING_BUDGET_THRESHOLD` (Sprint 14) and the 1,000-hour incidence-rate reliability floor (Sprint 15) are both documented as deliberate conventions rather than hidden magic numbers.

**Consequence:** `database/models/alerts.py`'s `ProjectAlertSent` is a genuinely new, small table — `project_id`, `alert_type` (`budget_variance` | `safety_warning`), `last_status_value`, `last_sent_at`, with a unique constraint on `(project_id, alert_type)` so each project has at most one row per alert type (an upsert, not an append-only log — this table answers "when did we last alert on this," not "give me a full alert history," which Deliverable 4's optional alert-history view reads from the same row rather than needing a separate audit trail). `app/services/alert_service.py`'s decision function is pure and testable without Celery or a real email send: given the current computed status and the last-sent row (or none), it returns whether to fire and, if so, updates the row — the same "pure decision logic, database read/write kept at the edges" shape `cost_service.py` and `safety_trend_service.py` already established.
